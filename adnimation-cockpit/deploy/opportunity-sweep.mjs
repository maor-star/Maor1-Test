...
async function main() {
  ...
  const threads = await sql`
    select thread_id, subject, snippet, counterpart_name, counterpart_email,
           known_contact, known_company, last_from_me, last_message_at
    from mail_threads
    where last_from_me = false
      and last_message_at >= now() - ${`${DAYS} days`}::interval
  ...
  console.log(
    `read ${threads.length} conversations in ${Math.round((Date.now() - started) / 1000)}s. ` +
      `${matched} looked like opportunities, ${proposed} were new. ` +

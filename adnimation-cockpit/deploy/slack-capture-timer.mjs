...
const service = `[Unit]
Description=Capture opportunities sent to the cockpit bot in Slack
...
ExecStart=/usr/bin/node /opt/cockpit-jobs/slack-capture.mjs
TimeoutStartSec=600
SuccessExitStatus=0 78
`;

const timer = `[Unit]
Description=Slack capture, every ten minutes
...
const commands = [
  'set -e',
  `cat > /etc/systemd/system/slack-capture.service <<'UNIT'\n${service}UNIT`,
  `cat > /etc/systemd/system/slack-capture.timer <<'UNIT'\n${timer}UNIT`,
  'systemctl daemon-reload',
  'systemctl enable --now slack-capture.timer',
  'systemctl list-timers slack-capture.timer --no-pager | head -2',
];

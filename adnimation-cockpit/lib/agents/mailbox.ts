...
export interface MailFacts {
  ...
  /** Gmail's own category labels, which are evidence but not a verdict. */
  labels: string[];
  /** Whether the sender is someone the company actually deals with. */
  knownContact: boolean;
  /** Whether he has ever replied to this sender. */
  everReplied: boolean;
  ageHours: number;
}
...
const PROMO_SIGNALS: [RegExp, string][] = [
...
export function looksPromotional(mail: MailFacts): PromoGuess {
...
export function isSpentAuthCode(mail: MailFacts, expiryHours = CODE_EXPIRY_HOURS): CodeGuess {
  const text = `${mail.subject ?? ''}\n${mail.snippet ?? ''}`;

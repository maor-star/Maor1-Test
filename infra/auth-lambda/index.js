...
<table role="presentation" width="680" style="max-width:680px;width:100%;background:#fff;border-radius:16px;overflow:hidden;border:1px solid #e2e8f0" cellpadding="0" cellspacing="0">
  ...
  <tr><td style="padding:4px 16px 12px"><table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="font-size:14px">${topRows || '<tr><td style="padding:8px;color:#64748b">אין הוצאות בשבוע זה</td></tr>'}</table>
    ${Array.isArray(w.pending) && w.pending.length ? `<div style="margin-top:8px;font-size:12.5px;color:#64748b;background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;padding:8px 12px">⏳ לא נכללו בדוח — אישורים זמניים של חברת האשראי שעדיין לא נסלקו (${fmtILS(w.pendingSum)}): ${w.pending.slice(0, 5).map((p) => escH(p.name) + ' ' + fmtILS(p.value) + ' (' + fmtD(p.d) + ')').join(' · ')}. הם ייכללו כשיהפכו לחיוב סופי.</div>` : ''}</td></tr>

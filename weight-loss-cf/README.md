...
- **אין אזור מאמן ואין משתמש מנהל.** נרשמים דרך מסך ההרשמה, וכל מי שנרשם מצטרף לקבוצה.
...
→ <http://127.0.0.1:8787>, ואז נרשמים דרך מסך ההרשמה.
...
├── migrations/             מיגרציות D1 (סכמה, תגים ומאמרים)
├── src/
...
שמירה ושליפה של תמונה מ-R2, וכל בדיקות ההרשאות (משתמש אחר 403, אנונימי 401, עוגייה מזויפת נדחית, השבתת חשבון מנתקת סשן קיים).
אחרי העיצוב החדש נבדקו גם היעדים בשליטת המשתמש, היקף המותניים, מסך הקבוצה וספריית המאמרים.

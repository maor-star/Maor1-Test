...
const ROUTES = [
  ...
  { path: 'science', title: 'המדע', render: viewScience },
  { path: 'articles', title: 'מאמרים', render: viewArticles },
...
async function render() {
  ...
  try {
    ...
    else if (route) await route.render(el);
...
function setupAuthScreen() {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      state.me = await api('/auth/' + mode, { method: 'POST', body: Object.fromEntries(new FormData(form).entries()) });
      form.reset();
      showApp();
